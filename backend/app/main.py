...
def call_clova(text: str, category: str, urgency: str):
    ...
    cache_key = f"clova:draft:{hashlib.md5(f'{text}:{category}:{urgency}'.encode()).hexdigest()}"
    if _redis:
        try:
    ...
    try:
        ...
        if _redis:
            try:
                _redis.setex(cache_key, REDIS_CACHE_TTL, content)
            except Exception:
                pass
...
def is_daily_limit_exceeded() -> bool:
    try:
        with get_db() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT COALESCE(SUM(total_cost_krw), 0) FROM cost_daily WHERE date = CURRENT_DATE"
                )
                row = cur.fetchone()
                return bool(row and row[0] >= COST_ALERT_THRESHOLD)
    except Exception:
        return False
...
def create_ticket(request: TicketRequest):
    ...
    elif is_daily_limit_exceeded():
        backend = "template-fallback"
        draft = build_template_draft(category, has_pii, urgency)
        send_telegram(f"💰 [한도초과 폴백] 일일 CLOVA 비용 한도(₩{COST_ALERT_THRESHOLD}) 초과 → 템플릿 자동 전환")
    else:
        clova_draft, latency_ms = call_clova(masked, category, urgency)
        if clova_draft:
            backend = "clova"
            draft = clova_draft
...
@app.post("/tickets/{ticket_id}/regenerate-draft")
def regenerate_draft(ticket_id: int):
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT masked_text, category, urgency, has_pii FROM tickets WHERE id = %s",
                (ticket_id,),
            )
            row = cur.fetchone()
    if not row:
        raise HTTPException(status_code=404, detail="티켓을 찾을 수 없습니다.")
    masked_text, category, urgency, has_pii = row
    draft, latency_ms = call_clova(masked_text, category, urgency)
    if draft:
        backend = "clova"
    else:
        draft = build_template_draft(category, has_pii, urgency)
        backend = "template"
    return {"draft": draft, "backend": backend, "latency_ms": latency_ms}
...
def telegram_send(body: TelegramSendRequest):
    if not TELEGRAM_TOKEN or not TELEGRAM_CHAT_ID:
        raise HTTPException(status_code=503, detail="Telegram 설정이 없습니다.")
    send_telegram(body.message)
    return {"ok": True}
...
def send_ticket_response(ticket_id: int, body: SendResponseRequest):
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT id, category, urgency, masked_text FROM tickets WHERE id = %s",
                (ticket_id,),
            )
            row = cur.fetchone()
    ...
    if not TELEGRAM_TOKEN or not TELEGRAM_CHAT_ID:
        raise HTTPException(status_code=502, detail="Telegram이 설정되지 않았습니다.")

    t_id, category, urgency, masked_text = row
    msg = (
        f"📨 [헬프데스크] 상담원 응답 발송\n"
        f"문의 ID: #{t_id}\n"
        f"카테고리: {category} · 긴급도: {urgency}\n\n"
